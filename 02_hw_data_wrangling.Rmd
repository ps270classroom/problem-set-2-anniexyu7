...
total_candidates <- nrow(fb_ads)
print(paste("There are", total_candidates, "candidates."))

knitr::kable(incumbency_table, col.names = c("Incumbency Status", "Number of Candidates"))
...
Democratic Incumbents were the type of candidate that sponsored the most promoting ads. 
...
party_incumbent_promote <- fb_ads %>%
  ...
  arrange(desc(promote_prop))
 

knitr::kable(party_incumbent_promote, 
             digits = 3, 
             col.names = 
               c("Party", "Incumbent Status", "Self-Promotion Ad Proportion"))

kable(party_incumbent_promote)
```
...
Most of the House candidates did not have more than 10 million ad impressions on Facebook. 
...
```{r}
fb_ads <- read.csv("data/fb_ads.csv")
fb_ads$impressions <- as.numeric(as.character(fb_ads$impressions))
...
ggplot(data = plot_q3, aes(x = impressions_millions)) +
  geom_histogram(binwidth = 1) +
  labs(title = "Histogram of Impressions in the Millions (US House)", 
...
fb_top_plot <- ggplot(data = fb_top_plot2, 
                      aes(
